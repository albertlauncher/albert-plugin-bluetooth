...
void BluetoothDevice::setName(const QString &name)
{
    ...
    DEBG << QString("Bluetooth device name changed to '%1' (%2)")
                .arg(name, d->address_);
...
void BluetoothDevice::setState(State s)
{
    ...
    DEBG << QString("Bluetooth device state changed to '%1' (%2 %3)")
...
void BluetoothDevice::connectionCallback(std::optional<QString> error)
{
    if (error)
    {
        WARN << QString("Failed to connect to device: '%1'. Error: '%2'")
